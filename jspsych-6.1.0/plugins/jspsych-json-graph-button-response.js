...
jsPsych.plugins["json-graph-button-response"] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'json-graph-button-response',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if (Array.isArray(trial.button_html)) {
      if (trial.button_html.length == trial.choices.length) {
        ...
        console.error('Error in json-graph-button-response plugin. The length of the button_html array does not equal the length of the choices array');
      }
    } else {
    ...
    html += '<div id="jspsych-json-graph-button-response-btngroup">';

    for (var i = 0; i < trial.choices.length; i++) {
      var str = buttons[i].replace(/%choice%/g, trial.choices[i]);
      html += '<div class="jspsych-json-graph-button-response-button" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'" id="jspsych-json-graph-button-response-button-' + i +'" data-choice="'+i+'">'+str+'</div>';
    }
    html += '</div>';
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      display_element.querySelector('#jspsych-json-graph-button-response-button-' + i).addEventListener('click', function(e){
    ...
    function after_response(choice) {
      ...
      var btns = document.querySelectorAll('.jspsych-json-graph-button-response-button button');
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#jspsych-json-graph-button-response-stimulus').style.visibility = 'hidden';
