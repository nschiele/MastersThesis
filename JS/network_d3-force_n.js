...
function renderNLGraph(jsonFileData, hl){
...
// for each node create a g element to contain both the circle element and the text element
...
var circles  = node.append("circle")
		.attr("stroke", function(d){
			for(var h = 0; h < highlights.length; h++){
				...
				if(d.id.localeCompare(hl[0]) === 0 || d.id.localeCompare(hl[1]) === 0){
					return  ColorHighlights ? "#000" : "#fff";
				}
			}
			return "#fff";
		})
		...
		.attr("fill", function(d){
			for(var h = 0; h < highlights.length; h++){
				var hl = highlights[h].split("-");
				if(d.id.localeCompare(hl[0]) === 0 || d.id.localeCompare(hl[1]) === 0){
					return  ColorHighlights ? "#ff0000" : "#00ACD9";
				}
			}
			return "#00ACD9";
		})
...
// add tooltip to each node group
node.append("title")
    .text(function(d) {
			return d.id; });
